-- MIGRACIÓN CORREGIDA PARA STAGING - Versión 2
-- Primero actualizar datos, luego cambiar constraints

-- 1. Actualizar datos existentes ANTES de cambiar constraints
UPDATE projects SET status = 'in_progress' WHERE status = 'active';
UPDATE projects SET status = 'completed' WHERE status NOT IN ('in_progress', 'completed');

-- 2. Ahora sí cambiar el constraint de status
...
-- 5. Agregar constraints para expenses (sin IF NOT EXISTS)
DO $$
BEGIN
...
-- 6. Actualizar función para recalcular costos (incluir net_amount y tax_amount)
...
-- 7. Trigger para auto-calcular net_amount y tax_amount si no se proporcionan
...
-- 8. Calcular net_amount y tax_amount para gastos existentes
UPDATE expenses 
SET 

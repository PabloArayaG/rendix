-- MIGRACIÓN FINAL PARA STAGING - Versión 4
-- Sin deshabilitar triggers del sistema

-- 1. Actualizar datos existentes ANTES de cambiar constraints
...
-- 4. Actualizar tabla expenses para coincidir con tipos TypeScript
DO $$
BEGIN
...
-- 5. Calcular net_amount y tax_amount para gastos existentes
UPDATE expenses 
SET 
...
-- 6. Agregar constraints para expenses
DO $$
BEGIN
...
-- 7. Eliminar triggers personalizados existentes antes de recrear
DROP TRIGGER IF EXISTS trigger_expenses_update_project_costs_insert ON expenses;
DROP TRIGGER IF EXISTS trigger_expenses_update_project_costs_update ON expenses;
DROP TRIGGER IF EXISTS trigger_expenses_update_project_costs_delete ON expenses;
DROP TRIGGER IF EXISTS trigger_calculate_expense_amounts ON expenses;

-- 8. Recrear función para recalcular costos (versión segura)
...
BEGIN
    ...
    -- Actualizar proyecto con costos reales (sin trigger recursivo)
    UPDATE projects SET
...
-- 10. Recrear triggers de forma segura
